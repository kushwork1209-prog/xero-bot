...
class Backup(commands.GroupCog, name="backup"):
    ...
    async def list_backups(self, interaction: discord.Interaction):
        ...
        if not backups:
            return await interaction.response.send_message(embed=info_embed("No Backups", "No backups found. Create one with `/backup create`."), ephemeral=True)
    ...
    async def restore(self, interaction: discord.Interaction, backup_id: int):
        ...
        await interaction.followup.send(embed=success_embed("Backup Restored!", f"Settings from backup **#{backup_id}** ({backup['backup_name']}) have been restored."), ephemeral=True)
    ...
    async def delete(self, interaction: discord.Interaction, backup_id: int):
        async with aiosqlite.connect(self.bot.db.db_path) as db:
            ...
            await db.commit()
        await interaction.response.send_message(embed=success_embed("Deleted", f"Backup #{backup_id} has been deleted."), ephemeral=True)
    ...
    async def export(self, interaction: discord.Interaction, backup_id: int):
        ...
        await interaction.followup.send(embed=success_embed("Exported", f"Backup #{backup_id} exported as `{fname}`."), file=file, ephemeral=True)
