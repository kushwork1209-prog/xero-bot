...
class Branding(commands.GroupCog, name="branding"):
    ...
    async def set_banner(self, interaction: discord.Interaction, image: discord.Attachment):
        ...
        embed = success_embed("Unified Branding Active", f"This image is now the pride of your server and will be used across:\n\n{module_list}")
    ...
    async def view_branding(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🎨 Embed Color", value=settings.get("embed_color", "#00D4FF"), inline=True)
        
        channel_id = settings.get("banner_channel_id")
        channel_mention = f"<#{channel_id}>" if channel_id else "Not Set"
        embed.add_field(name="📂 Storage Channel", value=channel_mention, inline=True)
        
        image_url = settings.get("unified_image_url")
        embed.add_field(name="🖼️ Unified Image", value="Active" if image_url else "None", inline=True)
